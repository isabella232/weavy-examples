(function ($) {
    ...
    this.Weavy = function () {
        ...
        function addAndRemoveBubbles(newBubbles) {
            if (newBubbles) {
                ...
                newBubbles.forEach(function (newBubble) {
                    ...
                    if ([].some.call(self.bubbles, function (b) { return b.space_id == newBubble.space_id })) {
        ...
        function conversationRead(id) {
            this.unreadConversations = this.unreadConversations.map(function (unreadConversation) { return unreadConversation.id !== id });
            updateConversations.call(this, this.unreadConversations);
        }

        function appendConversation(conversation) {
            // remove if already in list - will be added again
            this.unreadConversations = this.unreadConversations.map(function (unreadConversation) { return unreadConversation.id !== conversation.id });
            ...
            switch (e.data.name) {
                ...
                case "request-disconnect":
                    if (e.source !== undefined) {
                        ...
                        var bubble = [].filter.call(self.bubbles, function (b) { return b.space_id == spaceId }).pop();
        ...
        weavy.realtime.on("bubbleopened", function (e, data) {
            ...
            if (data.isMessenger) {
                ...
                if (data.type === "personal" && [].some.call(self.bubbles, function (b) { return b.space_id == data.space_id && b.type === "global"; })) {
                    data.type = "global";
                }
        ...
        weavy.realtime.on("bubbleremoved", function (e, data) {
            ...
            self.bubbles = [].filter.call(self.bubbles, function (bubble) {
